...
async function createConnection(options = {}) {
    ...
    console.log(`[CONN] WA Web version: ${version.join('.')}`);
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (qr && !state.creds.registered) {
            if (usePairingCode) {
                // ─── PAIRING CODE MODE (Custom Code Supported!) ───
                console.log('\n[PAIRING] 📱 PAIRING CODE MODE');
                console.log('[PAIRING] Custom pairing code supported!');
                console.log('[PAIRING] Enter code in WhatsApp: Settings → Linked Devices → Link with Phone Number\n');
                ...
                if (cleanNumber && cleanNumber.length >= 10) {
                    try {
                        // @qwerty-xcv/baileys supports custom pairing code!
                        // Format: requestPairingCode(phoneNumber, customCode)
                        const customCode = config.pairingCode || undefined;

...
function togel(jid, betAmount, chosenNumbers) {
    const user = getUser(jid);
    const cost = betAmount || 100;
    
    if ((user.koin || 0) < cost) return { success: false, reason: 'insufficient', balance: user.koin || 0 };
    
    // Generate random winning number (4 digits)
    const winNum = String(Math.floor(Math.random() * 10000)).padStart(4, '0');
    const winDigits = winNum.split('').map(Number);
    
    // Player picks 2 digits (kop + ekor)
    const playerNum = chosenNumbers ? String(chosenNumbers).padStart(2, '0').slice(0, 2) : String(Math.floor(Math.random() * 100)).padStart(2, '0');
    const playerDigits = playerNum.split('').map(Number);
    ...
    // Exact 4 digit match (Jackpot)
    if (playerNum === winNum.slice(2)) {
        prize = cost * 3000;
        resultType = '🏆 JACKPOT 4D!';
        prizeMultiplier = 3000;
    }
    // 3 digit match (Last 3)
    else if (playerNum === winNum.slice(1)) {
        prize = cost * 500;
        resultType = '🥇 3D MATCH!';
        prizeMultiplier = 500;
    }
    // 2 digit match (Ekor)
    else if (playerNum === winNum.slice(2)) {
        prize = cost * 70;
        resultType = '🥈 2D MATCH (Ekor)!';
        prizeMultiplier = 70;
    }
    // Kop match (2nd digit)
    else if (playerDigits[0] === winDigits[1]) {
        prize = cost * 15;
        resultType = '🥉 KOP MATCH!';
        prizeMultiplier = 15;
    }
    // Kepala match (3rd digit)
    else if (playerDigits[1] === winDigits[2]) {
        prize = cost * 10;
        resultType = '🥉 KEPALA MATCH!';
        prizeMultiplier = 10;
    }
    //colok bebas (any digit matches)
    else if (playerDigits.some(d => winDigits.includes(d))) {
        prize = cost * 2;
        resultType = '🎯 COLOK BEBAS!';
        prizeMultiplier = 2;
    }
    // No match
    else {
    ...
    user.exp = (user.exp || 0) + (prize > 0 ? 20 : 5);
    saveUser(jid, user);
...
function togelInfo() {
    return {
        rules: [
            'Pilih 2 angka (00-99)',
            'Bandar generate 4 angka random',
            'Match = menang!'
        ],
        prizes: [
            { match: '2D Ekor (angka 3-4)', multiplier: 'x70', example: 'Pilih 56, Result 1256 → 56=70x' },
            { match: 'Kop (angka ke-2)', multiplier: 'x15', example: 'Pilih 2x, Result 1234 → 2=15x' },
            { match: 'Kepala (angka ke-3)', multiplier: 'x10', example: 'Pilih x3, Result 1234 → 3=10x' },
            { match: 'Colok Bebas (1 angka cocok)', multiplier: 'x2', example: 'Pilih 1x, Result 1234 → 1=2x' },
            { match: 'No Match', multiplier: 'x0', example: 'Kehilangan bet' },
        ],
        tips: [
            '🎯 Colok bebas = peluang menang lebih tinggi',
            '💰 2D Ekor = hadiah besar tapi sulit',
            '🎲 Gunakan angka keberuntungan!' 
        ]
    };
}

// ═══════════════════════════════════════════════
// 17. TOGEL CHALLENGE (PvP)
// ═══════════════════════════════════════════════

function togelChallenge(challengerJid, targetJid, amount) {
    const challenger = getUser(challengerJid);
    const target = getUser(targetJid);
    
    if ((challenger.koin || 0) < amount) return { success: false, reason: 'challenger_insufficient' };
    if ((target.koin || 0) < amount) return { success: false, reason: 'target_insufficient' };
    
    // Both pick numbers
    const cNum = String(Math.floor(Math.random() * 100)).padStart(2, '0');
    const tNum = String(Math.floor(Math.random() * 100)).padStart(2, '0');
    
    // Generate winning number
    const winNum = String(Math.floor(Math.random() * 10000)).padStart(4, '0');
    
    // Calculate results
    const cResult = checkTogelMatch(cNum, winNum);
    const tResult = checkTogelMatch(tNum, winNum);
    
    // Determine winner
    let winner, loser;
    if (cResult.prize > tResult.prize) {
        winner = challengerJid;
        loser = targetJid;
    } else if (tResult.prize > cResult.prize) {
        winner = targetJid;
        loser = challengerJid;
    } else {
        // Draw - no money exchange
        return {
            success: true,
            draw: true,
            winNumber: winNum,
            challenger: { number: cNum, result: cResult },
            target: { number: tNum, result: tResult },
        };
    }
    ...
    saveUser(winner, winnerUser);
    saveUser(loser, loserUser);
...
function checkTogelMatch(playerNum, winNum) {
    const pDigits = playerNum.split('').map(Number);
    const wDigits = winNum.split('').map(Number);
    
    if (playerNum === winNum.slice(2)) return { prize: 3000, type: '2D Ekor', match: true };
    if (playerNum === winNum.slice(1)) return { prize: 500, type: '3D', match: true };
    if (pDigits[0] === wDigits[1]) return { prize: 15, type: 'Kop', match: true };
    if (pDigits[1] === wDigits[2]) return { prize: 10, type: 'Kepala', match: true };
    if (pDigits.some(d => wDigits.includes(d))) return { prize: 2, type: 'Colok Bebas', match: true };
    return { prize: 0, type: 'No Match', match: false };
}
